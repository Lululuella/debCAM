...
#'     The default is 20.
#' @param lof.thres Remove local outlier using \code{\link[DMwR]{lofactor}} function.
...
#' The first loading vector is forced to be c(1,1,...,1) with unit norm normaliztion.
#' The remaining are eignvectors from PCA in the space orthogonal to the first vector.
#' Perspective projection is to project dimension-reduced gene expression vectors
#' to the hyperplane orthogonal to c(1,0,...,0), i.e., the first axis in the new coordinate system.
#' local outlier removal is optional to exclude outliers in simplex formed after perspective projection.
#' Finallly, gene expression vectors are aggregated by clustering
#' to further reduce the impact of noise/outler and help improve the efficiency of simplex corner detction.
...
#' \item{c.outlier}{The clusters with the gene number smaller than MG.num.thres.}
#' \item{centers}{The centers of corner clusters (candidate clusters containing marker genes).}
#' @export
#' @examples
#' #obtain data
#' data(ratMix3)
...
CAMPrep <- function(data, dim.rdc = 10, thres.low = 0.05, thres.high = 0.95,
    ...
    Valid <- sigNorm > quantile(sigNorm, thres.low) & sigNorm <= quantile(sigNorm, thres.high)
    X <- X[,Valid]
    ...
    C1 <- P1 %*% Xmeanrm
    Xmeanrm <- Xmeanrm - matrix(rep(C1, 1, each=Lorg), Lorg) * matrix(P1, Lorg, dataSize)
