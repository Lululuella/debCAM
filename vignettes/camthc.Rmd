...
Convex Analysis of Mixtures (CAM) is a fully unsupervised computational method to analyze tissue samples composed of unknown numbers and varying proportions of distinct subpopulations. CAM assumes that the measured expression level is the weighted sum of each subpopulation’s expression, where the contribution from a single subpopulation is proportional to the abundance and specific expression of that subpopulation. This linear mixing model can be formulated as $\mathbf{X'=AS'}$. CAM can can identify molecular markers directly from the original mixed expression matrix, $\mathbf{X}$, and further estimate the constituent proportion matrix, $\mathbf{A}$, as well as subpopulation-specific expression profile matrix, $\mathbf{S}$.
...
#running CAM
rCAM <- CAM(data, K = 4, thres.low = 0.70, thres.high = 0.95, seed = 111)
...
[GSE41826](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE41826) conducted a reconstitution experiment by mixing neuron and glial derived DNA from a single individual from 10% ~ 90%. The code chunk below shows how to use CAM to blindly seperate 9 mixtures into neuron and glial specific CpG methylation quantification. Additional requirments of running CAM on methylation data: 
...
gsm<- getGEO('GSE41826')
mat <- exprs(gsm)
colnames(mat)<- gsm$title
mat.mix<- mat[,paste0('Mix',1:9)]
...
#annot <- annot[rownames(mat.mix),]
#mat.mix <- mat.mix[annot$CHR != 'X' & annot$CHR != 'Y',]

#downsample CpG sites
SampleId <- sample(seq_len(nrow(mat.mix)), 20000)

#running CAM
rCAM <- CAM(mat.mix[SampleId,], K = 2, thres.low = 0.10, thres.high = 0.60)
Aest <- rCAM$ASestResult$'2'$Aest
Atrue <- cbind(seq(0.1, 0.9, 0.1), seq(0.9, 0.1, -0.1))
cor(Aest, Atrue)

#obtain a full list of marker CpG sites
MGstat <- MGstatistic(mat.mix, Aest)
MGlist.FC <- lapply(1:2, function(x) 
    rownames(MGstat)[MGstat$idx == x & MGstat$OVE.FC > 10])
```
...
```{r, echo=TRUE, message=FALSE, eval=FALSE}
rCAM <- CAM(1 - mat.mix[SampleId,], K = 2, thres.low = 0.10, thres.high = 0.60)
```
